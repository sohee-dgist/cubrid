...
namespace predication
{
    ...
    void PredExprCompiler::compile_internal(const PARSER_CONTEXT *ctx, PT_NODE *pt_node)
    {
        if (pt_node == nullptr)
        {
            return;
        ...
        {
            compile_internal(ctx, get_left_child(pt_node));
        }

        if (has_right_child(pt_node))
        {
            compile_internal(ctx, get_right_child(pt_node));
        }

        compile_unit(ctx, pt_node);
    }
    ...
    */
    InstNode
    PredExprCompiler::pt_node_to_instnode(const PARSER_CONTEXT *ctx, PT_NODE *node)
    {
        InstNode inst_node;
    ...
    void PredExprCompiler::compile_unit(const PARSER_CONTEXT *ctx, PT_NODE *node)
    {
        script_->get_inst_nodes()->push_back(pt_node_to_instnode(ctx, node));
