...
namespace predication
{
    ...
    class CompiledPredExpr
    {
        public:
            ...
            int get_last_inst_idx()
            {
                return last_inst_idx_;
            }
            int get_last_regu_var_index()
            {
                return last_regu_var_index_;
            }
            int get_last_bool_index()
            {
    ...
    {
        public:
            PredExprCompiler(const PARSER_CONTEXT *ctx, PT_NODE *pt_node) : ctx_(ctx), pt_node_(pt_node), script_(new CompiledPredExpr()) {}
            CompiledPredExpr *compile()
            {
                compile_internal(ctx_, pt_node_);
        ...
        private:
            ...
            const PARSER_CONTEXT *ctx_;
            PT_NODE *pt_node_;
            std::stack<PreInstNode> pre_inst_nodes_;
            InstNode pt_node_to_instnode(const PARSER_CONTEXT *ctx, PT_NODE *node);
            void compile_internal(const PARSER_CONTEXT *ctx, PT_NODE *pt_node);
            void compile_unit(const PARSER_CONTEXT *ctx, PT_NODE *node);
    };
