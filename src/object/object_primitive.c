      ...
      else if (copy)
	{
	  ...
	  if (set == NULL)
	  ...
	  else
	    {
	      ...
	      if (ref == NULL)
		{
		  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, sizeof (DB_COLLECTION));
		  return ER_FAILED;
		}
	      else
		{
			  ...
			  int copy_buf_len)
{
  ...
  if (!copy)
      ...
      if (new_ == NULL)
	{
	  ...
	  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, size);
	  return ER_FAILED;
	}
      else
	{
...
mr_data_readmem_string (OR_BUF * buf, void *memptr, TP_DOMAIN * domain, int size)
{
  ...
  if (memptr == NULL)
      ...
      if (size)
	{
	  ...
	  if (new_ == NULL)
	    {
	      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, mem_length);
	      return;
	    }
	  else
	    {
